...
function _execute(n=8, thickness=Length / 2 / 100, distortion=0.0, bl=0.0, visualize=false)
    ...
    vtkwrite("cos_2t_p_hyp_clamp-$(n)-h=$(thickness)-d=$(distortion)-bl=$(bl)-m.vtu", fens, fes; scalars=scalars, vectors=[("u", dchi.values[:, 1:3])])
    scalars = []
    for nc in 1:3
    ...
    vtkwrite("cos_2t_p_hyp_clamp-$(n)-h=$(thickness)-d=$(distortion)-bl=$(bl)-n.vtu", fens, fes; scalars=scalars, vectors=[("u", dchi.values[:, 1:3])])
    scalars = []
    for nc in 1:2
    ...
    vtkwrite("cos_2t_p_hyp_clamp-$(n)-h=$(thickness)-d=$(distortion)-bl=$(bl)-q.vtu", fens, fes; scalars=scalars, vectors=[("u", dchi.values[:, 1:3])])
...
let
    ...
    for bl in [0.0, 1.0]
        for distortion in [1.0, 0.0]
            ...
            display(ax)
            pgfsave("cos_2t_p_hyp_clamp_d=$(distortion)_bl=$(bl)-convergence.pdf", ax)
