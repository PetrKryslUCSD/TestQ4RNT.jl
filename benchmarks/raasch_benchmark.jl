...
function _execute(input = "raasch_s4_1x9.inp", drilling_stiffness_scale = 1.0, visualize = true, nL = 9, nW = 1)
    ...
    @info "$(nL)x$(nW) mesh: $(round(targetu, digits=8)), error $(round(1 - targetu/bench_sol, digits = 4)*100)%"
...
using .raasch_benchmark
raasch_benchmark.test_convergence()

all_drilling_stiffness_scale, all_results = raasch_benchmark.test_dep_drilling_stiffness_scale()
...
let
    ...
    display(ax)
    pgfsave("raasch_benchmark-dependence-on-drilling_stiffness_scale.pdf", ax)
end