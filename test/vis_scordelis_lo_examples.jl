...
function _execute_dsg_model(n = 8, visualize = true)
    ...
    if visualize
        ...
        vtkwrite("vis_scordelis_lo-$(n)-m.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
        scalars = []
        for nc in 1:3
        ...
        vtkwrite("vis_scordelis_lo-$(n)-n.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
        scalars = []
        for nc in 1:2
        ...
        vtkwrite("vis_scordelis_lo-$(n)-q.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])

        # vtkwrite("vis_scordelis_lo-$(n)-uur.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3]), ("ur", dchi.values[:, 4:6])])
...
function test_convergence(ns = [4, 6, 8, 16, 32], visualize = false)
    ...
    results = []
    for n in ns
        v = _execute_dsg_model(n, visualize)
